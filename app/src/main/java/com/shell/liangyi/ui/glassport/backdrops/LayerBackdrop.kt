...
import com.shell.liangyi.ui.glassport.internal.InverseLayerScope
...
class LayerBackdrop internal constructor(
    ...
    override fun DrawScope.drawBackdrop(
        ...
        val sourceCoordinates = coordinates ?: return
        val backdropCoordinates = layerCoordinates ?: return
        withTransform({
            if (layerBlock != null) {
            ...
            val offset = try {
                backdropCoordinates.localPositionOf(sourceCoordinates)
            } catch (_: Exception) {
                sourceCoordinates.positionInWindow() - backdropCoordinates.positionInWindow()
            }
    ...
    private fun obtainInverseLayerScope(): InverseLayerScope {
        return inverseLayerScope?.apply { reset() } ?: InverseLayerScope().also { inverseLayerScope = it }
    }
}
