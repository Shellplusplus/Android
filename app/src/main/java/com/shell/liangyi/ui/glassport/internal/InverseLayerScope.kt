...
internal class InverseLayerScope : GraphicsLayerScope {
    ...
    private fun DrawTransform.inverseTransformAtTopLeft(
        ...
        val reusableMatrix = matrix ?: Matrix().also { matrix = it }
        if (reusableMatrix.values.size < 16) return
        ...
        if (det == 0f) return
        val invDet = 1f / det
        reusableMatrix[0, 0] = a11 * invDet
        reusableMatrix[0, 1] = -a01 * invDet
        reusableMatrix[1, 0] = -a10 * invDet
        reusableMatrix[1, 1] = a00 * invDet
        transform(reusableMatrix)
    }
}
