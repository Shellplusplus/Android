...
@Composable
fun rememberShellBlurBackdrop(enableBlur: Boolean = true): LayerBackdrop? {
...
fun BackdropEffectScope.lens(
    ...
    val shader = if (useDispersion) {
        ROUNDED_RECT_REFRACTION_WITH_DISPERSION_SHADER
    } else {
        ROUNDED_RECT_REFRACTION_SHADER
    }
    val key = if (useDispersion) "ShellGlassLensDispersion" else "ShellGlassLens"

    val scale = downscaleFactor.coerceAtLeast(1).toFloat()
    val scaledSizeWidth = size.width / scale
    val scaledSizeHeight = size.height / scale
    val scaledPadding = padding / scale
    val scaledRefractionHeight = refractionHeight / scale
    val scaledRefractionAmount = refractionAmount / scale
